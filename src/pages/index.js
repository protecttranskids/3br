    ...
    <div>
      ...
      <div className="scroll-area" style={{ padding: '20px 16px' }}>
        ...
        <div className="card" style={{ padding: 16, marginBottom: 16, textAlign: 'center' }}>
          ...
          <a href="https://docs.google.com/forms/d/e/1FAIpQLSeOB2Ja3-lXhHQojmSldI1O0pBAwLPiYMKBCXTI4QV-eSMwxA/viewform" target="_blank" rel="noopener noreferrer" style={{ display: 'block', padding: '12px', borderRadius: 12, background: 'var(--accent)', color: '#fff', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>Share Feedback</a>
        </div>
      </div>
    </div>
  );
...
function UserProfile({ userId, onClose, onBookTap }) {
  ...
  var readCount = shelves.filter(function(s) { return s.shelf === 'read'; }).length;
    ...
    <div className="overlay" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="scroll-area" style={{ padding: 20, flex: 1 }}>
        ...
        {recSets.length === 0 ? (
          ...
          recSets.map(function(rs) {
            var src = rs.source_book;
            var coverUrl = src && src.isbn ? 'https://covers.openlibrary.org/b/isbn/' + src.isbn + '-S.jpg' : null;
            return (
              <div key={rs.id} className="card" style={{ padding: 14, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                <div onClick={function() { onBookTap && onBookTap(src); }} style={{ flexShrink: 0, cursor: 'pointer' }}>
                  {coverUrl ? (
                    <img src={coverUrl} alt="" style={{ width: 44, height: 64, borderRadius: 6, objectFit: 'cover', background: 'var(--border)' }} />
                  ) : (
                    <div style={{ width: 44, height: 64, borderRadius: 6, background: 'var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{'📖'}</div>
        ...
        {shelves.length > 0 && (
          <>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {shelves.slice(0, 12).map(function(s) {
                var bk = s.books;
                var cv = bk && bk.isbn ? 'https://covers.openlibrary.org/b/isbn/' + bk.isbn + '-S.jpg' : null;
                return (
                  <div key={s.id} onClick={function() { onBookTap && onBookTap(bk); }} style={{ cursor: 'pointer' }}>
                    {cv ? (
